...
(function () {
  ...
  if (p.startsWith('/cliente-jescri/') || p.startsWith('/area-do-cliente/') || p.startsWith('/clientes/')) {
...
(function () {
  ...
  try {
    ...
    if (cfg && cfg.site && cfg.site.navClientLabel) {
      ...
      Array.from(document.querySelectorAll('a[href="/area-do-cliente/"], a[href="/clientes/"], a[href="/clientes"]')).forEach((a) => {
        a.textContent = label;
        a.setAttribute('href', '/clientes/');
      });
    }
  } catch (e) {}
  ...
  // - inicializa quando o render terminar (ela:accordion-ready)
  // -----------------------------
  const initAccordion = () => {
  ...
  document.addEventListener('ela:accordion-ready', initAccordion);
